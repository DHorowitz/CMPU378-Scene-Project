/* ------------------------------------------------------------------
*     April 4th            Graphics Scene Graph Assignment
*     Student: Dylan Horowtiz           Prof: Tom Ellman
*     NOTE: Colors will not change unless accessed first
--------------------------------------------------------------------*/
...
void enableClippingPlanes() {
	GLdouble equation[4] = { 100.0,0.0,0.0, 0.0 };
...
void displayScene()
{
	...
	glutSolidTorus(1.0, 2.0, resolution, resolution);
	glPopMatrix();

	//Draw Sphere
	glPushMatrix();
	glTranslatef(0.0, 3.0, 1.0);
	gluSphere(quadratic, 1.0, resolution, resolution);
	glPopMatrix();
	...
	gluCylinder(quadratic, 1.0, 1, 3, resolution, resolution);
	glPopMatrix();
	...
	glRotatef(120.0, 1.0, 0.0, 0.0);
	glTranslatef(0.0, 3.0, 0.0);
	glutSolidCone(2.0, 10.0, resolution, resolution);
	glPopMatrix();
	...
	glBegin(GL_QUADS);
	//bl
	glVertex3f(-4.0, -1, 1.0);
	//br
	glVertex3f(4.0, -1, 1.0);
	//tr
	glVertex3f(4.0, 8, -1.0);
	//tl
	glVertex3f(-4.0, 8, -1.0);
	glEnd();
	glPopMatrix();
	...
	gluDisk(quadratic, 2.0, 5.0, resolution, resolution);
	glPopMatrix();

	//Draw Clippping Plane
	glPushMatrix();
	glTranslatef(2.0, 1.0, 0.0);
	enableClippingPlanes();
	glutSolidSphere(7.0, resolution, resolution);
	disableClippingPlanes();
	glPopMatrix();

	//Draw Second Clipping Plane
	glPushMatrix();
	glTranslatef(-2.0, 1.0, 0.0);
...
int main(int argc, char** argv)
{
	...
	glutCreateWindow("Scene Project");
